...
public class WallsStatsDisplayer
  extends StatsDisplayer
{
  public WallsStatsDisplayer(String name)
  {
    super(name);
    getStats();
  }
  ...
  public void displayStatsInChat()
  {
    ...
    new ChatMessageComposer(this.playerName + "'s stats in " + GameType.WALLS.getName(), EnumChatFormatting.GREEN).send();
  ...
  private void getStats()
  {
    try
    {
      this.coins = this.statistics.get("Walls").getAsJsonObject().get("coins").getAsInt();
    }
    catch (Exception e) {}
    try
    {
      this.kills = this.statistics.get("Walls").getAsJsonObject().get("kills").getAsInt();
    }
    catch (Exception e) {}
    try
    {
      this.wins = this.statistics.get("Walls").getAsJsonObject().get("wins").getAsInt();
    }
    catch (Exception e) {}
  }
}
